...
void MPCUtils::convertWaypointsToMPCTraj(const common_msgs::Trajectory &lane, MPCTrajectory &mpc_traj)
{
  mpc_traj.clear();
  const double k_tmp = 0.0;
  const double t_tmp = 0.0;
  for (const auto &wp : lane.points)
  {
    const double x = wp.x;
    const double y = wp.y;
    const double z = 0;
    const double yaw = wp.angle;
    const double vx = wp.velocity;
    mpc_traj.push_back(x, y, z, yaw, vx, k_tmp, t_tmp);
  }
}

void MPCUtils::convertWaypointsToMPCTrajWithDistanceResample(const common_msgs::Trajectory &path, const std::vector<double> &path_time,
                                                             const double &dl, MPCTrajectory &ref_traj)
{
  ...
  {
    double dx = path.points.at(i).x - path.points.at(i - 1).x;
    double dy = path.points.at(i).y - path.points.at(i - 1).y;
...
void MPCUtils::convertWaypointsToMPCTrajWithTimeResample(const common_msgs::Trajectory &path, const std::vector<double> &path_time,
...
void MPCUtils::convertWaypointsToMPCTrajWithResample(const common_msgs::Trajectory &path, const std::vector<double> &path_time,
                                                     const std::vector<double> &ref_index, const double &d_ref_index, MPCTrajectory &ref_traj)
{
  ...
  while (point < ref_index.back())
  {
    ...
    pos0.position.x = path.points.at(j - 1).x;
    pos0.position.y = path.points.at(j - 1).y;
    pos0.position.z = 0;
    tf::Quaternion quaternion = tf::createQuaternionFromRPY(0, 0, path.points.at(j-1).angle);
    quaternionTFToMsg(quaternion, pos0.orientation);

     geometry_msgs::Pose pos1;
    pos1.position.x = path.points.at(j).x;
    pos1.position.y = path.points.at(j).y;
    pos1.position.z = 0;
    tf::Quaternion quaternion1 = tf::createQuaternionFromRPY(0, 0, path.points.at(j).angle);
    quaternionTFToMsg(quaternion1, pos1.orientation);  
     geometry_msgs::Twist twist0;
    twist0.linear.x = path.points.at(j - 1).velocity;
     geometry_msgs::Twist twist1;
    twist1.linear.x = path.points.at(j).velocity;
...
void MPCUtils::calcPathRelativeTime(const common_msgs::Trajectory &path, std::vector<double> &path_time)
{
  double t = 0.0;
  path_time.clear();
  ...
  {
    const double x0 = path.points.at(i).x;
    const double y0 = path.points.at(i).y;
    const double z0 = 0;
    const double x1 = path.points.at(i + 1).x;
    const double y1 = path.points.at(i + 1).y;
    const double z1 = 0;
    const double dx = x1 - x0;
    const double dy = y1 - y0;
    const double dz = z1 - z0;
    const double dist = sqrt(dx * dx + dy * dy + dz * dz);
    double v = std::max(double(std::fabs(path.points.at(i).velocity)), 1.0);
    t += (dist / v);
    path_time.push_back(t);
  }
}
