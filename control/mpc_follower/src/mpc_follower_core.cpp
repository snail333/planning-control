    ...
    : nh_(""), pnh_("~"), my_position_ok_(false), my_velocity_ok_(false), my_steering_ok_(false)
{
  ...
  pnh_.param("vehicle_model_driver_ratio", driver_ratio_, 15.4);
  /* vehicle model setup */
  ...
  pnh_.param("in_selfpose_name", in_selfpose, std::string("localization"));
  pnh_.param("in_vehicle_status_name", in_vehicle_status, std::string("pcican"));
  pub_twist_cmd_ = nh_.advertise<geometry_msgs::TwistStamped>(out_twist, 1);
  pub_steer_vel_ctrl_cmd_ = nh_.advertise<common_msgs::SystemOverall>(out_vehicle_cmd, 1);
...
void MPCFollower::timerCallback(const ros::TimerEvent &te)
{
  ...
  if (vehicle_model_ptr_ == nullptr || qpsolver_ptr_ == nullptr)
  {
   ...
   // publishControlCommands(0, 0,0, steer_cmd_prev_, 0);
        double temptorque = 0;
	publishControlCommands(0, 0,temptorque, steer_cmd_prev_, 0);
    return;
  }
  ...
  const bool mpc_solved = calculateMPC(vel_cmd, acc_cmd,torque,steer_cmd, steer_vel_cmd);
...
publishControlCommands(vel_cmd, acc_cmd, torque, steer_cmd, steer_vel_cmd);
};

bool MPCFollower::calculateMPC(double &vel_cmd, double &acc_cmd,double &torque, double &steer_cmd, double &steer_vel_cmd)
{
  ...
  //acc_cmd = (ref_traj_.vx[2] - ref_traj_.vx[1]) / DT;//del by wly

  steer_cmd_prev_ = steer_cmd;
  pid_.setTargetSpeed(vel_cmd);
  acc_cmd = pid_.Speed_PID_Acc();
  torque = pid_.Speed_PID_AccAndTorque(acc_cmd);
  /* save to buffer */
...
void MPCFollower::callbackRefPath(const common_msgs::Trajectory::ConstPtr &msg)
{
  current_waypoints_ = *msg;
  DEBUG_INFO("[MPC] path callback: received path size = %lu", current_waypoints_.points.size());
  MPCTrajectory traj;
...
void MPCFollower::callbackPose(const common_msgs::localizationConstPtr& msg)
{
  ...
  vehicle_status_.pose = current_pose_;
  pid_.setCurrentSpeed(msg->speed);
  pid_.setCurrentPitch(msg->pose.rotation.pitch);

  my_position_ok_ = true;
};

void MPCFollower::callbackVehicleStatus(const common_msgs::CanRecieve &msg)
{
  vehicle_status_.tire_angle_rad = amathutils::deg2rad(msg.vehicleInfo.steerangle/driver_ratio_);
  vehicle_status_.twist.linear.x = amathutils::kmph2mps(msg.vehicleInfo.speed);
  my_steering_ok_ = true;
  my_velocity_ok_ = true;
};

void MPCFollower::publishControlCommands(const double &vel_cmd, const double &acc_cmd, const double& lon_torque,
                                         const double &steer_cmd, const double &steer_vel_cmd)
{
  common_msgs::SystemOverall ccs;
  ccs.header.stamp = ros::Time::now();
  ccs.systemstate.latcontrol.targetangle = amathutils::rad2deg(steer_cmd*driver_ratio_);
  ccs.systemstate.latcontrol.targettorque = 0;
  ccs.systemstate.loncontrol.targetspeed = vel_cmd;
  ccs.systemstate.loncontrol.targetaccelation = acc_cmd;
  ccs.systemstate.loncontrol.targettorque = lon_torque;
  pub1_.publish(ccs);

  std_msgs::Float32 msg;
  msg.data = vel_cmd* 3.6;
  pub19_.publish(msg);
}
...
MPCFollower::~MPCFollower()
{
  ...
  publishControlCommands(vel_cmd, acc_cmd,torque_cmd, steer_cmd, steer_vel_cmd);
};
