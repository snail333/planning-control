...
private:
  ...
  common_msgs::Trajectory current_waypoints_;                    //!< @brief current waypoints to be followed
  ...
  // class
  waypoint_follower::LongitudinalControl pid_;
  ...
  double wheelbase_, driver_ratio_;                //!< @brief vehicle wheelbase length [m] to convert steering angle to angular velocity
 ...
 // void callbackPose(const geometry_msgs::PoseStamped::ConstPtr &);
   ...
   * @param [in] goaltorque goaltorque command [N] for vehicle control
   ...
   */
void publishControlCommands(const double &vel_cmd, const double &acc_cmd, const double &goaltorque,const double &steer_cmd, const double &steer_vel_cmd);
   ...
   */
  bool calculateMPC(double &vel_cmd, double &acc_cmd,double &torque, double &steer_cmd, double &steer_vel_cmd);
  ...
  ros::Publisher pub1_,pub19_;                          //!< @brief publisher for debug info
