  ...
  : nh_(nh), pnh_(pnh), is_initialized_(false)
{
  ...
  pnh_.param<double>("wheel_base", wheelbase_, 3.0);
  pnh_.param<double>("driver_ratio", driver_ratio_, 20);
...
void VehicleModelROS::setVehicleCmd(const common_msgs::SystemOverallConstPtr& msg)
{
  ...
  else if (vehicle_model_type_ == VehicleModelType::IDEAL_STEER || vehicle_model_type_ == VehicleModelType::DELAY_STEER)
  {
    ...
    input << msg->systemstate.loncontrol.targetspeed, amathutils::deg2rad(msg->systemstate.latcontrol.targetangle / driver_ratio_); //m/s,前轮转角
    vehicle_model_ptr_->setInput(input);
  }
  else
  {
...
void VehicleModelROS::setInitialState(const geometry_msgs::Pose& pose, const geometry_msgs::Twist& twist)
{
  ...
  const double steer = (fabs(vx) > LINEAR_VEL_MIN) ? atan(wheelbase_ * wz / vx) : 0.0;  //前轮转角
...
{
  return current_steering_angle_ * driver_ratio_;
}
