...
visualization_msgs::Marker displayExpandWaypoints(const std::vector<common_msgs::TrajectoryPoint>& waypoints, int size)
{
  visualization_msgs::Marker marker;
  ...
  for (auto& el : marker.points)
  {
    ...
    el.x = waypoints[waypoints.size() - idx - 1].x;
    el.y = waypoints[waypoints.size() - idx - 1].y;
  }
