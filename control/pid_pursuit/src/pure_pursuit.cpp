...
double PurePursuit::calcCurvature(geometry_msgs::Point target) const
{
  ...
  else
  {
    if (numerator > 0)
    {
      kappa = KAPPA_MIN_;   //RADIUS_MAX_(9e10)
    }
    else
    {
      kappa = -KAPPA_MIN_;
  ...
  int next_waypoint, geometry_msgs::Point* next_target) const
{
  ...
  next_pt.x = current_waypoints_.at(next_waypoint).x;
  next_pt.y = current_waypoints_.at(next_waypoint).y;
  geometry_msgs::Point prenext_pt;
  prenext_pt.x = current_waypoints_.at(next_waypoint - 1).x;
  prenext_pt.y = current_waypoints_.at(next_waypoint - 1).y;
  ...
  geometry_msgs::Point end = next_pt;
  geometry_msgs::Point start = prenext_pt;
...
void PurePursuit::getNextWaypoint()
{
  ...
  for (int i = 0; i < path_size; i++)
  {
    ...
    geometry_msgs::Point targetwp;
    targetwp.x = current_waypoints_.at(i).x;
    targetwp.y = current_waypoints_.at(i).y;
    // if there exists an effective waypoint
    if (getPlaneDistance(targetwp, current_pose_.position) > lookahead_distance_)
    {
      next_waypoint_number_ = i;
      return;
    }
...
bool PurePursuit::canGetCurvature(double* output_kappa)
{
  ...
  for (const auto& el : current_waypoints_)
  {
    ...
    target.x = el.x;
    target.y = el.y;
    if (getPlaneDistance(target, current_pose_.position)
  ...
  {
    next_target_position_.x = current_waypoints_.at(next_waypoint_number_).x;
    next_target_position_.y = current_waypoints_.at(next_waypoint_number_).y;
