...
void createGlobalLaneArrayVelocityMarker(const common_msgs::TrajectoryArray& lane_waypoints_array)
{
  ...
  int count = 1;
  for (auto lane : lane_waypoints_array.trajectories)
  {
    ...
    for (int i = 0; i < static_cast<int>(lane.points.size()); i++)
    {
      ...
      point_pose.position.x = lane.points[i].x;
      point_pose.position.y = lane.points[i].y;
      point_pose.position.z = 0;
      tf::Quaternion quaternion = tf::createQuaternionFromRPY(0, 0, lane.points[i].angle);
      quaternionTFToMsg(quaternion, point_pose.orientation);

      velocity_marker.pose.position = calcAbsoluteCoordinate(relative_p, point_pose);
      velocity_marker.pose.position.z += 0.2;

      // double to string
      std::string vel = std::to_string(mps2kmph(lane.points[i].velocity));
...
void createGlobalLaneArrayChangeFlagMarker(const common_msgs::TrajectoryArray& lane_waypoints_array)
{
  visualization_msgs::MarkerArray tmp_marker_array;
  // display by markers the velocity of each waypoint.
  ...
  for (auto lane : lane_waypoints_array.trajectories)
  {
    ...
    for (int i = 0; i < static_cast<int>(lane.points.size()); i++)
    {
      ...
      marker.pose.position = calcAbsoluteCoordinate(relative_p, point_pose);
      marker.pose.position.z += 0.2;

      // double to string
      std::string str = "";
      str = "S";
      // if (lane.points[i].change_flag == static_cast<ChangeFlagInteger>(ChangeFlag::straight))
      // {
      //   str = "S";
      // }
      // else if (lane.points[i].change_flag == static_cast<ChangeFlagInteger>(ChangeFlag::right))
      // {
      //   str = "R";
      // }
      // else if (lane.points[i].change_flag == static_cast<ChangeFlagInteger>(ChangeFlag::left))
      // {
      //   str = "L";
      // }
      // else if (lane.points[i].change_flag == static_cast<ChangeFlagInteger>(ChangeFlag::unknown))
      // {
      //   str = "U";
      // }
...
void createLocalWaypointVelocityMarker(std_msgs::ColorRGBA color, int closest_waypoint,
                                       const common_msgs::Trajectory& lane_waypoint)
{
  // display by markers the velocity of each waypoint.
  ...
  for (int i = 0; i < static_cast<int>(lane_waypoint.points.size()); i++)
  {
    velocity.id = i;
    geometry_msgs::Point relative_p;
    relative_p.y = -0.65;
    geometry_msgs::Pose point_pose;
    point_pose.position.x = lane_waypoint.points[i].x;
    point_pose.position.y = lane_waypoint.points[i].y;
    point_pose.position.z = 0;
    tf::Quaternion quaternion = tf::createQuaternionFromRPY(0, 0, lane_waypoint.points[i].angle);
    quaternionTFToMsg(quaternion, point_pose.orientation);
    
    velocity.pose.position = calcAbsoluteCoordinate(relative_p, point_pose);
    velocity.pose.position.z += 0.2;

    // double to string
    std::ostringstream oss;
    oss << std::fixed << std::setprecision(1) << mps2kmph(lane_waypoint.points[i].velocity);
...
/*void createGlobalLaneArrayMarker(std_msgs::ColorRGBA color, const common_msgs::TrajectoryArray& lane_waypoints_array)
{
...
void createGlobalLaneArrayOrientationMarker(const common_msgs::TrajectoryArray& lane_waypoints_array)
{
  visualization_msgs::MarkerArray tmp_marker_array;
  visualization_msgs::Marker lane_waypoint_marker;
  lane_waypoint_marker.header.frame_id = "map";
  ...
  for (auto lane : lane_waypoints_array.trajectories)
  {
    ...
    for (int i = 0; i < static_cast<int>(lane.points.size()); i++)
    {
      ...
      lane_waypoint_marker.pose = point_pose;
...
void createLocalPathMarker(std_msgs::ColorRGBA color, const common_msgs::Trajectory& lane_waypoint)
{
  visualization_msgs::Marker lane_waypoint_marker;
  lane_waypoint_marker.header.frame_id = "map";
  lane_waypoint_marker.header.stamp = ros::Time::now();
  ...
  for (unsigned int i = 0; i < lane_waypoint.points.size(); i++)
  {
    geometry_msgs::Point point;
    point.x = lane_waypoint.points[i].x;
    point.y = lane_waypoint.points[i].y;
    point.z = 0;
    lane_waypoint_marker.points.push_back(point);
  }
...
void createLocalPointMarker(const common_msgs::Trajectory& lane_waypoint)
{
  visualization_msgs::Marker lane_waypoint_marker;
  lane_waypoint_marker.header.frame_id = "map";
  lane_waypoint_marker.header.stamp = ros::Time::now();
...
void laneArrayCallback(const common_msgs::TrajectoryArrayConstPtr& msg)
{
  ...
  publishMarkerArray(g_global_marker_array, g_global_mark_pub, false);
}

void finalCallback(const common_msgs::TrajectoryConstPtr& msg)
{
  ...
  publishMarkerArray(g_local_waypoints_marker_array, g_local_mark_pub, false);
}
