...
void WaypointLoaderNode::initPubSub()
{
  ...
  // setup publisher
  lane_pub_ = nh_.advertise<common_msgs::TrajectoryArray>("/based/lane_waypoints_raw", 10, true);
}
...
void WaypointLoaderNode::run()
{
  ...
  common_msgs::TrajectoryArray lane_array;
...
void WaypointLoaderNode::createLaneArray(const std::vector<std::string>& paths, common_msgs::TrajectoryArray* lane_array)
{
  for (const auto& el : paths)
  {
    common_msgs::Trajectory lane;
    createLaneWaypoint(el, &lane);
    lane_array->trajectories.emplace_back(lane);
  }
}

void WaypointLoaderNode::createLaneWaypoint(const std::string& file_path, common_msgs::Trajectory* lane)
{
  ...
  std::vector<common_msgs::TrajectoryPoint> wps;
...
void WaypointLoaderNode::loadWaypointsForVer1(const char* filename, std::vector<common_msgs::TrajectoryPoint>* wps)
{
  ...
  for (size_t i = 0; i < wps->size(); ++i)
  {
    if (i != last)
    {
      double yaw = atan2(wps->at(i + 1).y - wps->at(i).y,
                         wps->at(i + 1).x - wps->at(i).x);
      wps->at(i).angle = yaw;
    }
    else
    {
      wps->at(i).angle = wps->at(i - 1).angle;
    }
  }
}

void WaypointLoaderNode::parseWaypointForVer1(const std::string& line, common_msgs::TrajectoryPoint* wp)
{
  std::vector<std::string> columns;
  parseColumns(line, &columns);

  wp->x = std::stod(columns[0]);
  wp->y = std::stod(columns[1]);
  wp->velocity = kmph2mps(std::stod(columns[3]));
}

void WaypointLoaderNode::loadWaypointsForVer2(const char* filename, std::vector<common_msgs::TrajectoryPoint>* wps)
{
  std::ifstream ifs(filename);
...
void WaypointLoaderNode::parseWaypointForVer2(const std::string& line, common_msgs::TrajectoryPoint* wp)
{
  std::vector<std::string> columns;
  parseColumns(line, &columns);
  ...
  wp->x = std::stod(columns[0]);
  wp->y = std::stod(columns[1]);
  wp->angle = std::stod(columns[3]);
  wp->velocity = kmph2mps(std::stod(columns[4]));
}

void WaypointLoaderNode::loadWaypointsForVer3(const char* filename, std::vector<common_msgs::TrajectoryPoint>* wps)
{
  std::ifstream ifs(filename);
                                              ...
                                              common_msgs::TrajectoryPoint* wp)
{
  ...
  wp->angle = std::stod(map["yaw"]);
  wp->velocity = kmph2mps(std::stod(map["velocity"]));
